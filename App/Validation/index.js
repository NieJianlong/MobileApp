 ...
 */
const STRONG_PASSWORD_VALIDATION =
  /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})/;
...
export const isValidEmail = (loginInput) => {
  return EMAIL_REG.test(loginInput);
};

export const isValidPhone = (loginInput) => {
  return INTNTL_PHONE_REG.test(loginInput);
};
